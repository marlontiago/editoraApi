...
{
    // (opcional) bloqueia pré-visualização se o pedido estiver cancelado/finalizado
    if (in_array($pedido->status, ['cancelado', 'finalizado'], true)) {
        return back()->with('error', 'Não é possível emitir pré-visualização para pedidos cancelados/finalizados.');
    }
    ...
    DB::transaction(function () use ($pedido, $proximoNumero) {
        ...
        // NOVO: ao pré-visualizar, se ainda estiver em_andamento, vira pre_aprovado
        if ($pedido->status === 'em_andamento') {
            ...
            if (method_exists($pedido, 'registrarLog')) {
                $pedido->registrarLog('status_alterado', 'Status alterado para Pré-aprovado (pré-visualização de nota).', []);
            }
        }

        // 🔒 unicidade de "emitida": cancela emitidas anteriores
        ...
        $nota = NotaFiscal::create([
            ...
            'pedido_snapshot'       => [
                ...
                'status'             => $pedido->status, // já estará "pre_aprovado"
