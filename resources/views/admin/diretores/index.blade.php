<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold text-gray-800 leading-tight">Diretores Comerciais</h2>
                <p class="text-sm text-gray-500 mt-1">Cadastro e informações de contato.</p>
            </div>

            <a href="{{ route('admin.diretor-comercials.create') }}"
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($diretores as $d)
                            <tr class="hover:bg-gray-50/60">
                                <td class="px-4 py-3 font-medium text-gray-900">{{ $d->nome }}</td>

                                <td class="px-4 py-3">
                                    @if(!empty($d->email))
                                        <a href="mailto:{{ $d->email }}" class="text-blue-700 hover:underline">
                                            {{ $d->email }}
                                        </a>
                                    @else
                                        —
                                ...
                                <td class="px-4 py-3">{{ $d->telefone ?: '—' }}</td>

                                <td class="px-4 py-3">
                                    {{ $d->cidade ?: '—' }} / {{ $d->estado ? strtoupper($d->estado) : '—' }}
                                </td>

                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.diretor-comercials.show', $d) }}"
                                        ...
                                        <a href="{{ route('admin.diretor-comercials.edit', $d) }}"
                                        ...
                                        <form action="{{ route('admin.diretor-comercials.destroy', $d) }}"
                                              method="POST"
                                              class="inline"
                        ...
                        @empty
                            <tr>
                                <td class="px-4 py-10 text-center text-sm text-gray-500" colspan="5">
                                    Nenhum registro.
                                </td>
                            </tr>
                        @endforelse
