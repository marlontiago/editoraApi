...
class NotaPagamentoController extends Controller
{
    ...
    public function store(Request $request, NotaFiscal $nota)
    {
        ...
        // ===== Regra de percentuais (PRIORIDADE: cidade vigente → global ATIVO vigente → global vigente → cadastro)
        $cidadeIds = $nota->pedido->cidades->pluck('id');
    ...
    private function resolvePercentualByCidade($owner, $cidadeIds, Carbon $refDate): float
    {
        ...
        if ($cidadeIds->isNotEmpty()) {
            ...
            if ($cityVigentes->isNotEmpty()) {
                return (float) $cityVigentes->first()->percentual_vendas; // mais recente
            }
        }
        ...
        if ($globalVigentes->isNotEmpty()) {
            return (float) $globalVigentes->first()->percentual_vendas;
        }
