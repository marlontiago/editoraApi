...
{
    // precisa das cidades para aplicar a regra de contrato por cidade
    $nota->load([
    ...
    // aplica a mesma regra do store para sugerir os % na tela
    $cidadeIds = $nota->pedido->cidades->pluck('id');

    $percGestor       = $this->resolvePercentualByCidade(optional($nota->pedido)->gestor, $cidadeIds);
    $percDistribuidor = $this->resolvePercentualByCidade(optional($nota->pedido)->distribuidor, $cidadeIds);
    ...
    public function store(Request $request, NotaFiscal $nota)
{
    ...
    // Precisamos das cidades e dos anexos para aplicar a regra por cidade
    $nota->load([
        'pedido.gestor.anexos',
        'pedido.distribuidor.anexos',
        'pedido.cidades',
    ...
    if (($data['diretor_id'] ?? null) && ($data['perc_comissao_diretor'] === null)) {
        ...
        if ($auto !== null) $data['perc_comissao_diretor'] = (float) $auto;
    }
    ...
    // ===== Regra: contrato por cidade > global ativo > cadastro
    $cidadeIds = $nota->pedido->cidades->pluck('id');

    $percGestor       = $this->resolvePercentualByCidade(optional($nota->pedido)->gestor, $cidadeIds);
    $percDistribuidor = $this->resolvePercentualByCidade(optional($nota->pedido)->distribuidor, $cidadeIds);

    // Comissões (sobre o líquido)
    ...
    DB::transaction(function () use (
        ...
        NotaPagamento::create([
            ...
            // Snapshot após aplicar a regra por cidade
    ...
    {
        if ($pagamento->nota_fiscal_id !== $nota->id) abort(404);
    ...
    private function resolvePercentualByCidade($owner, $cidadeIds): float
{
    if (!$owner) return 0.0;

    $base = $owner->anexos(); // relação morphMany já existente

    // 1) contrato por cidade ATIVO
    $byCityActive = (clone $base)
        ...
        ->first();
    if ($byCityActive && $byCityActive->percentual_vendas !== null) {
        return (float) $byCityActive->percentual_vendas;
    }

    // 2) contrato por cidade MAIS RECENTE (mesmo não ativo)
    $byCityAny = (clone $base)
        ->where('tipo', 'contrato_cidade')
        ->whereIn('cidade_id', $cidadeIds)
        ->orderByDesc('data_assinatura')
        ->orderByDesc('created_at')
        ->first();
    if ($byCityAny && $byCityAny->percentual_vendas !== null) {
        return (float) $byCityAny->percentual_vendas;
    }

    // 3) global ATIVO (sem cidade)
    $globalActive = (clone $base)
        ->whereNull('cidade_id')
        ...
        ->orderByDesc('created_at')
        ->first();
    if ($globalActive && $globalActive->percentual_vendas !== null) {
        return (float) $globalActive->percentual_vendas;
    }

    // 4) fallback: percentual do cadastro
