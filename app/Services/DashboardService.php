...
class DashboardService
{
    ...
    {
        return min(max((int) $request->integer('per_page', 20), 5), 100);
    }

    public function resolveOrderBy(Request $request): string
    {
        $allowed = ['id','data','created_at','valor_total'];
        return in_array($request->get('order_by'), $allowed, true) ? $request->get('order_by') : 'id';
    }

    public function resolveDir(Request $request): string
    {
        return $request->get('dir') === 'asc' ? 'asc' : 'desc';
    }

    public function validateDashboardFilters(Request $request): array
    {
        return $request->validate([
            ...
            'cidades.*'       => ['integer','exists:cities,id'],
        ]);
    }

    public function getUiCombos(): array
    {
        $gestoresList = Gestor::with('user:id,name')
            ->orderBy('razao_social')
            ->get(['id','user_id','razao_social']);

        $distribuidoresList = Distribuidor::with('user:id,name')
            ->orderBy('razao_social')
            ->get(['id','user_id','razao_social']);

        $cidadesList = City::orderBy('name')->get(['id','name']);

        return compact('gestoresList','distribuidoresList','cidadesList');
    }

    public function getTopKpis(): array
    {
        ...
        return compact('totalProdutos','totalGestores','totalUsuarios');
    }

    public function getEstoqueBaixo(): mixed
    {
    ...
    public function getEstoqueEmRiscoPedidosEmAndamento(): mixed
    {
    ...
    public function getContratosAlertas(): array
    {
        ...
        return compact('gestoresVencendo','gestoresVencidos','distribuidoresVencendo','distribuidoresVencidos');
    }

    public function rangeToDates(?string $ini, ?string $fim): array
    {
    ...
    /**
     * Monta a query base de pedidos + filtros (igual seu controller).
     */
    public function buildPedidosBaseQuery(array $filters)
    {
        $dataInicio     = $filters['data_inicio'] ?? null;
        $dataFim        = $filters['data_fim'] ?? null;
        $gestorId       = $filters['gestor_id'] ?? null;
        $distribuidorId = $filters['distribuidor_id'] ?? null;
        $status         = $filters['status'] ?? null;
        $cidadesIds     = $filters['cidades'] ?? [];
        ...
        if (!empty($cidadesIds)) {
            $baseQuery->whereHas('cidades', fn($q) => $q->whereIn('cities.id', $cidadesIds));
        }

        return $baseQuery;
    }

    public function computeAggregates($baseQuery): array
    {
        $sub = (clone $baseQuery)
        ...
        return compact('totalPedidosPeriodo','somaPeriodo');
    }

    public function paginatePedidos($baseQuery, int $perPage, string $orderBy, string $dir, Request $request)
    {
    ...
    public function somaGeralPedidos(): float
    {
        return Cache::remember(
            'dash.soma_geral_pedidos',
            now()->addMinutes(5),
            fn () => (float) Pedido::query()->sum('valor_total')
        );
    }

    /**
     * Retorna payload completo do dashboard (view).
     */
    public function getDashboardPayload(Request $request): array
    {
        $perPage = $this->resolvePerPage($request);
        $orderBy = $this->resolveOrderBy($request);
        $dir     = $this->resolveDir($request);

        $kpis = $this->getTopKpis();

        $produtosComEstoqueBaixo       = $this->getEstoqueBaixo();
        $estoqueParaPedidosEmPotencial = $this->getEstoqueEmRiscoPedidosEmAndamento();

        $contratos = $this->getContratosAlertas();

        $combos = $this->getUiCombos();

        $filters = $this->validateDashboardFilters($request);

        $baseQuery = $this->buildPedidosBaseQuery($filters);

        $aggs = $this->computeAggregates($baseQuery);

        $pedidos = $this->paginatePedidos($baseQuery, $perPage, $orderBy, $dir, $request);
        $somaPagina = $pedidos->getCollection()->sum('valor_total');

        $somaGeralTodosPedidos = $this->somaGeralPedidos();
        ...
        return array_merge(
            [
                ...
                'orderBy'                       => $orderBy,
                'dir'                           => $dir,
                // para manter seus nomes atuais na view:
                'dataInicio'                    => $filters['data_inicio'] ?? null,
                'dataFim'                       => $filters['data_fim'] ?? null,
                'gestorId'                      => $filters['gestor_id'] ?? null,
                'distribuidorId'                => $filters['distribuidor_id'] ?? null,
    ...
    public function chartNotasPagas(Request $request): array
    {
    ...
    public function chartVendasPorGestor(Request $request): array
    {
        [$start, $end] = $this->rangeToDates($request->input('data_inicio'), $request->input('data_fim'));
    ...
    public function chartVendasPorDistribuidor(Request $request): array
    {
        [$start, $end] = $this->rangeToDates($request->input('data_inicio'), $request->input('data_fim'));
    ...
    public function chartVendasPorCliente(Request $request): array
    {
        [$start, $end] = $this->rangeToDates($request->input('data_inicio'), $request->input('data_fim'));
    ...
    public function chartVendasPorCidade(Request $request): array
    {
        [$start, $end] = $this->rangeToDates($request->input('data_inicio'), $request->input('data_fim'));
    ...
    /**
     * Query usada pelo export PDF/Excel (mesma regra do controller original).
     */
    public function queryPedidosForExport(Request $request)
    {
        $request->validate([
            'data_inicio'     => ['nullable', 'date'],
            'data_fim'        => ['nullable', 'date', 'after_or_equal:data_inicio'],
    ...
    public function resolveExportNames(?int $gestorId, ?int $distribuidorId): array
    {
        ...
        return compact('nomeGestor','nomeDistribuidor');
    }
}
